...
一人でずっとコンパイルエラーを解決したり、ノイズの原因追及をやっているのは疲れたので、まだまったく不完全ですが作業途中版を公開しました。
...
* godot_test_project/にGodotのプロジェクトがあります。Godotエディタで開けます。
* プロジェクトフォルダ直下でbuild.batをコマンドプロンプトで実行するとソースがコンパイルされ、GDJucy\build\gdjucy\Debug\gdjucy.dllが生成されます。godot_test_project\bin以下にコピーすると、
